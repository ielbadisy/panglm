...
nonlinear fixed-effects estimator. Finite-T incidental-parameter bias can be
relevant in short panels, so agreement with another implementation should
not be interpreted as eliminating that statistical concern.
...
are reported and omitted before fitting, and `na.exclude` is respected by
fitted values and residuals. Nontrivial observation weights and nonzero
offsets are rejected explicitly because the corresponding weighted
conditional panel estimators are not currently implemented.
